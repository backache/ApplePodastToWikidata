#Get  URL
#http://itunes.apple.com/lookup?id=Q130863691
$Q ='Q1797751','Q3027762','Q4728682'
#to do
...
function Send-Wikidata {
    ...
    $out = ""
    #$QID="Q130863691"
    Write-Host "Retrieve the wikidata item $QID and put in a variable called WD then show me what you got"
    ...
    Write-Host "Make a dictionary of the response from Wikidata"
    ...
    Write-Host "From the Wikidata Dictionary  extra the value of the newest web feed URL"
    $APPLEID = $WD.statements.p5842.value.content | Sort-Object -Property ID | Select-Object -first 1
    
    if ($APPLEID.length -eq 0) { 
        write-host "no valid apple ID found" -ForegroundColor yellow
        $name = $WD.labels.en
        $name = $name.replace('"', '&quot;')
        #$name = $name.replace('/', '&quot;')
        $nameencoded = [System.Web.HTTPUtility]::UrlPathEncode($name)
        if (($WD.labels.mul.length -eq 0) -and ($WD.labels.en.length -gt 0)) { $out += "%7C%7C$QID%7CLmul%7C%22" + $nameencoded + "%22%7C" }
        ...
        Write-Host "Go and get Apple's data from $URL"
        #try {
        $Response = Invoke-WebRequest -Uri "$URL" -UseBasicParsing -ContentType "application/xml"

        $Feed = $Response.Content | ConvertFrom-Json
        write-host "Length is $($feed.results.length)"
        write-host "RecordCount is $($feed.resultCount)"
        if ($feed.resultCount -eq 0) { write-host "Apple has no records for this" } else {
            ...
            write-host "get the largest number of podcasts the wikidata entry has"
            [int]$AppleAmount = $Feed.Results.trackcount
            [int]$WDepisodecount = 0
            write-host "Apple says it has $AppleAmount"
            [int]$BiggestWDamount = $AppleAmount 

            #- 1
            ForEach ($CountEntry in $WD.statements.p1113.value.content.amount) {
                $WDepisodecount=$WDepisodecount+1
                if ($CountEntry.length -gt 0) {
                    [int]$currentcount = $CountEntry.Substring(1) 
                    #[int]$BiggestWDamount = $AppleAmount
                    write-host("currentcount is $currentcount entry has a valid value of $countentry biggest so far is $BiggestWDamount")
                }
                if (($currentcount -gt $BiggestWDamount) -or ($currentcount -eq $BiggestWDamount)) { $BiggestWDamount = $currentcount } else { 
                    write-host "lower value will remove"
                    $WDepisodecount=$WDepisodecount-1
                    $out += "%7C%7C%2D$QID%7CP1113%7C$currentcount" 
                }
                write-host("Number of episode wd has $WDepisodecount ")
            }

            if (($WDepisodecount -lt 1) -and ($AppleAmount -gt 0)) { $out += "%7C%7C$QID%7CP1113%7C$AppleAmount%7CP585%7C%2b$($datestr)T00:00:00Z%2F11%7CS248%7CQ70058728" }
            
            #removed date retrieved %7CP813%7C%2b$($datestr)T00:00:00Z%2F11
            write-host "does it have multilanguage set"
            if ($feed.results.collectionName.length -ne 0) { $name = $feed.results.collectionName } else { $name = $WD.labels.en }
            $name = $name.replace('"', '&quot;')
            $name = $name.replace('/', '%2F')
            $nameencoded = [System.Web.HTTPUtility]::UrlPathEncode($name)
            write-host("encoded name is $nameencoded")
            if (($WD.labels.mul.length -eq 0) -and ($WD.labels.en.length -gt 0)) { $out += "%7C%7C$QID%7CLmul%7C%22" + $nameencoded + "%22%7C" } 


            write-host "does it have it a genre set?"
            if ($WD.statements.p136.value.content.length -eq 0) {
            ...
            write-host "does it have it a name to be removed?"
            if ($WD.statements.p2561.value.content.length -gt 0) {
                $name = $WD.statements.p2561.value.content
                $name = [System.Web.HTTPUtility]::UrlPathEncode($name)
                $out += "%7C%7C-$QID%7CP2561%7C%22$name%22"
            }
            write-host "does it have a logo set?"
            ...
            #write-host "does it have multilanguage set"
            #$en=$WD.labels.en
            #write-host "English is $en"
            #if (($WD.labels.mul.length -eq 0) -and  ($WD.labels.en.length -gt 0)) {$out+="||$QID|Lmul|%22$en%22|"}
            ...
            #}
            #catch {
            #    $errstring += "Cannot download podcast feed for $QID"
            #}
...
foreach ($Qitem in $Q) {
    ...
    write-host ("rate limit value $ratelimitcount")
    write-host("Out length $($output.length)")
    write-host("items processed $spawncount")
    if ($ratelimitcount -eq 20) {
...
if ($output.length -gt 62) { Start-Process "chrome.exe" "$output" } else { "nothing more to upload" }
write-host ($output)
#   Start-Process "chrome.exe" "$output"
